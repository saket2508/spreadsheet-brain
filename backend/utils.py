...
def detect_column_types(df: pd.DataFrame) -> Dict[str, str]:
    ...
    for col in df.columns:
        ...
        if ('percent' in col_lower or 'margin' in col_lower or 'growth' in col_lower or 
            'rate' in col_lower or col_lower.endswith('%') or 
            sample_values.str.contains('%').any() or 
            (df[col].dtype in ['float64', 'int64'] and df[col].max() <= 1.0 and df[col].min() >= 0)):
            column_types[col] = 'percentage'
            
        # Check for currency/monetary columns
        elif ('revenue' in col_lower or 'cost' in col_lower or 'profit' in col_lower or 
              'expense' in col_lower or 'income' in col_lower or 'price' in col_lower or
              sample_values.str.contains('$').any()):
        ...
        # Default to categorical or numeric
...
def extract_formula_info(value: str) -> Dict[str, Any]:
    ...
    functions = re.findall(r'\b(SUM|AVERAGE|COUNT|IF|VLOOKUP|INDEX|MATCH|SUMIF|COUNTIF)\b', value.upper())
    if functions:
        formula_info['functions'] = functions
        
    # Mathematical operations
    operations = []
    if '+' in value: operations.append('addition')
    if '-' in value: operations.append('subtraction')
    if '*' in value: operations.append('multiplication')
    if '/' in value: operations.append('division')
    if operations:
...
def dataframe_to_documents(df: pd.DataFrame) -> List[Dict]:
    ...
    for i, row in df.iterrows():
        ...
        metadata = {
            "row_index": i,
            "column_types": {col: column_types[col] for col in df.columns},
            "formula_info": formula_info,
            "categories": row_categories,
            "business_concepts": list(set(row_categories)),  # Remove duplicates
            "classification_explanation": explain_classification(row_categories, row_text),
            "business_hierarchy": get_business_concept_hierarchy()
        }
